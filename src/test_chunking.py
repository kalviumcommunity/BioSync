...
class ChunkingTests(unittest.TestCase):
    ...
    def test_paragraph_strategy_preserves_boundaries_and_source(self) -> None:
        ...
        self.assertEqual(chunks, [
            Chunk("policy.txt", 1, "First policy paragraph with the refund rule."),
            Chunk("policy.txt", 2, "Second paragraph explains the payment method."),
        ])
    ...
    def test_comparison_runs_both_strategies_on_same_source(self) -> None:
        ...
        self.assertTrue(all(chunk.source == "policy.txt" for chunks, _ in comparison.values() for chunk in chunks))
